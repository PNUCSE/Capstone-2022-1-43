			   ...
			   struct bio *bio)
{
	...
	case REQ_OP_READ:
		...
		break;
	case REQ_OP_WRITE:
        if (bio->bi_disk->disk_name[0] == 'd' && bio_flagged(bio, BIO_EXT4_FILE) && !dmz_is_seq(zone))
            printk("ext4 Flag : 1 | Zone ID : %d | Zone Type : RND | Chunk Number : %llx | Sector Number : %llx\n",
                    zone->id, dmz_bio_chunk(zmd, bio), bio->bi_iter.bi_sector);

        if (bio->bi_disk->disk_name[0] == 'd' && bio_flagged(bio, BIO_EXT4_FILE) && dmz_is_seq(zone))
            printk("ext4 Flag : 1 | Zone ID : %d | Zone Type : SEQ | Chunk Number : %llx | Sector Number : %llx\n",
                    zone->id, dmz_bio_chunk(zmd, bio), bio->bi_iter.bi_sector);
      
        if (bio->bi_disk->disk_name[0] == 'd' && bio_flagged(bio, BIO_EXT4_JRNL) && !dmz_is_seq(zone))
            printk("jbd2 Flag : 1 | Zone ID : %d | Zone Type : RND | Chunk Number : %llx | Sector Number : %llx\n",
                     zone->id, dmz_bio_chunk(zmd, bio), bio->bi_iter.bi_sector);

        if (bio->bi_disk->disk_name[0] == 'd' && bio_flagged(bio, BIO_EXT4_JRNL) && dmz_is_seq(zone))
            printk("jbd2 Flag : 1 | Zone ID : %d | Zone Type : SEQ | Chunk Number : %llx | Sector Number : %llx\n",
                     zone->id, dmz_bio_chunk(zmd, bio), bio->bi_iter.bi_sector);
